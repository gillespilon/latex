...
\makeglossaries
\GlsXtrLoadResources[
    src={~/documents/repositories/latex/pilon_gilles_glossary.bib},
    selection={recorded and deps},
    sort=letter-nocase,
    selection={all} % shows all glossary entries
]

% the above shows every single term in pilon_gilles_glossary.bib
% use the following instead when only list terms used in the document
...
Here is a single citation with postnote only. $C_\text{pm}$ is a complex subject (\cite[][1-10]{chan1988}).

\section{Inserting glossary terms}\label{sec:glossary_terms}

Here is a request to fill in \gls{endog}, \glspl{endog}, \gls{exog}, \glspl{exog}, \gls{machinelearning}, and \gls{seborrheickeratosis}.

For glossary terms use \gls{aad}, \gls{mad}, \gls{mae}, \gls{mse}, \gls{rmse}, \gls{anome}, \gls{iam}, \gls{pam}, \gls{edr}, \glsxtrlong{xdr}, \gls{pythonnamespace}, \gls{cybersecurity}.
