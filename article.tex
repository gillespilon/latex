...
\addbibresource{pilon_gilles.bib}
...
\epigraph{Remember, the objective is to take the right action on the process. No
credit is given for finding the `right numbers.'}{Donald J. Wheeler}
...
\subsection{Greek Unicode}
\label{sec:greek_unicode}
...
% The definitive book on MSA is by Wheeler \cite{wheeler2006}. The definitive book on SPC is by Wheeler \cite{wheeler2004}. The original article on Taguchi indices was by Russell Boyles \cite{boyles1991}. A very good article on the $C_\text{pm}$ index is found in Chen et.al. \cite{chan1988}. Gunter \cite{gunter1989}.

Here is a single citation without prenote and postnote. The definitive book on MSA is by Wheeler \cite{wheeler2006}.

Here is a single citation with prenote only. The definitive book on SPC is by \cite[Wheeler][]{wheeler2006}.

Here is a single citation with prenote and postnote. For further details on $C_\text{pm}$, \cite[see][pages 1-10]{chan1988}.
